...
class OntologyMapper:
    def __init__(self):
        ...
        if "coulumn_Names" not in ss:
            ss.coulumn_Names = ['celltypes', 'cell', 'cells', 'Zelltypen', 'Zelle', 'Zellen']
        if "map" not in ss:
            ss.map = False
    ...
    def _get_mapping(self):
            ...
            for df in dataframes:
                for name in ss.coulumn_Names:
                    ...
                    if name in df.columns:
                        ...
                        celltypes = celltypes + df[name].str.replace('[{}]'.format(string.punctuation), '').tolist()
        ...
        result = text2term.map_terms(celltypes, "CL", base_iris="http://purl.obolibrary.org/obo/CL",  use_cache=True)
...
def build_term_lists(result):
    ...
    for index, row in result.iterrows():
        ...
        if(term in ss.term_list):
            ...
            ss.mapped_term_list[term_index].append((row['Mapped Term Label'], row['Mapping Score'])) 
        else:
